...
class eeModel(nn.Module):
    ...
    def print_summary(self):
        basemodel_total_params = sum(p.numel()
                                     for p in self.baseModel.parameters())
        basemodel_trainable_params = sum(
            p.numel() for p in self.baseModel.parameters() if p.requires_grad)

        shortBranch_total_params = sum(p.numel()
                                       for p in self.shortBranch.parameters())
        shortBranch_trainable_params = sum(
            p.numel() for p in self.shortBranch.parameters() if p.requires_grad)

        longBranch_total_params = sum(p.numel()
                                      for p in self.longBranch.parameters())
        longBranch_trainable_params = sum(
            p.numel() for p in self.longBranch.parameters() if p.requires_grad)

        print("Number of base parameters: {}".format(basemodel_total_params))
        print("Number of short branch parameters: {}".format(
            shortBranch_total_params))
        print("Number of long branch parameters: {}".format(
            longBranch_total_params))
        print("Difference = {}".format(
            longBranch_total_params-shortBranch_total_params))
