        ...
        for epoch in range(self.NUM_EPOCHS):
            ...
            for i, data in enumerate(tLoader, 0):
                ...
                loss1, loss2 = self.criterion(
                    output1, labels.long()), self.criterion(output2, labels.long())

                loss1Total += loss1.item()
                loss2Total += loss2.item()
                totalLoss += 0.5*loss1.item() + 0.5*loss2.item()

                loss1.backward(inputs=list(self.net.baseModel.parameters(
                ))+list(self.net.shortBranch.parameters()), retain_graph=True)
                loss2.backward(inputs=list(self.net.longBranch.parameters()))

                _, predicted1 = torch.max(output1, 1)
                _, predicted2 = torch.max(output2, 1)

                acc1 = accuracy_score(labels.detach().cpu(
                ).numpy(), predicted1.detach().cpu().numpy())
                acc2 = accuracy_score(labels.detach().cpu(
                ).numpy(), predicted2.detach().cpu().numpy())
            ...
            if vLoader:
                with torch.no_grad():
                    ...
                    self.history["1"]["validation"]["loss"].append(
                        loss1Total_v)
                    self.history["1"]["validation"]["accuracy"].append(
                        acc1Total_v)
                    self.history["2"]["validation"]["loss"].append(
                        loss2Total_v)
                    self.history["2"]["validation"]["accuracy"].append(
                        acc2Total_v)
    ...
    def testingSummary(self, recorder, nBranches=2, overall=True):
        ...
        for i in range(nBranches):
            ...
            if countSamples != 0:
                ...
                print("Branch {}: Accuracy {:.2f}% with {:.2f}% of the samples".format(
                    i+1, acc*100, countSamples/overallCount*100))
            else:
                print("Branch {}: Got 0% of the samples".format(i+1))
            overallAcc += acc*countSamples
        if overall:
            print("Overall Weighted Accuracy: {:.2f}%".format(
                overallAcc/overallCount*100))
    ...
    def train(self, tLoader, vLoader=None):
        ...
        for epoch in range(self.NUM_EPOCHS):
            ...
            for i, data in enumerate(tLoader, 0):
                ...
                acc = accuracy_score(labels.detach().cpu(
                ).numpy(), predicted.detach().cpu().numpy())
                totalAcc += acc
    ...
    def infer(self, sLoader):
        ...
        with torch.no_grad():
            ...
            for inputs, gTruth in sLoader:
                ...
                tLoss += loss.item()
                acc += accuracy_score(gTruth.detach().cpu().numpy(),
